<div>
    <form action="{{route('standing-orders.store')}}" method="POST" class="flex flex-col gap-8">
        ...
        <div class="flex gap-4 flex-wrap justify-center mt-4">
            {{--customer dropdown--}}
            <x-form.customer-select has-wire/>
            {{--standing order activation/start date--}}
            <x-form.form-wrapper>
                <x-form.form-label id="start_from" text="Start From"/>
                <x-form.form-input type="date" id="start_from" name="start_from"
            ...
            <div class="flex gap-8 flex-wrap my-2 justify-center">
                ...
                @for($i = 0; $i<7;$i++)
                    <div
                        ...
                        <h4 class="font-bold text-accent text-center">{{\Illuminate\Support\Carbon::create()->startOfWeek()->addDays($i)->dayName}}</h4>
                        @foreach($products as $product)
                            <x-form.form-wrapper>
                                <x-form.form-label id="products[{{$i}}][{{$product->product_id}}]"
                                                   text="{{$product->product_name}} {{$product->product_weight_g}}g"/>
                            </x-form.form-wrapper>
                            <x-form.form-input type="number" id="products[{{$i}}][{{$product->product_id}}]"
                                               name="products[{{$i}}][{{$product->product_id}}]" placeholder="0"/>
                        @endforeach
                    </div>
                @endfor
            </div>
