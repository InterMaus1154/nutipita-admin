...
<div class="flex flex-col gap-4 ">
    <div class="flex gap-8 justify-center sm:justify-between 2xl:justify-evenly flex-wrap sm:grid grid-cols-3 items-center">
        ...
        <div class="flex flex-col gap-2 mt-8 sm:flex-row sm:justify-center sm:items-center">
            <flux:button :variant="$selectedType === \NutiPita\Enums\FinancialRecordType::INCOME ? 'primary' : 'filled'" class="sm:order-1 hidden! sm:block!" wire:click="toggleType('{{FinancialRecordType::INCOME}}')">
            ...
            />
            <flux:button
                :variant="$selectedType === \NutiPita\Enums\FinancialRecordType::EXPENSE ? 'primary' : 'filled'"
            ...
            <div class="flex gap-2 justify-between sm:hidden order-2 sm:order-none">
                <flux:button :variant="$selectedType === \NutiPita\Enums\FinancialRecordType::EXPENSE ? 'primary' : 'filled'" class="order-2 sm:order-1" wire:click="toggleType('{{FinancialRecordType::EXPENSE}}')">
                    <flux:icon.banknote-arrow-down />
                </flux:button>
                <flux:button :variant="$selectedType === \NutiPita\Enums\FinancialRecordType::INCOME ? 'primary' : 'filled'" wire:click="toggleType('{{FinancialRecordType::INCOME}}')">
                    <flux:icon.banknote-arrow-up />
                </flux:button>
            </div>
        </div>
