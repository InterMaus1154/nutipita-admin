...
<div class="{{$class}}">
    @php
        ...
        function getYears(){
            ...
            $range = Order::selectRaw('MIN(order_due_at) as first, MAX(order_due_at) AS last')->first();

            if(is_null($range->first)){
                return [now()->year];
            }

            return range(
                Format::getYearFromDate($range->first),
                Format::getYearFromDate($range->last)
            );
        }
    @endphp
    <div class="relative"
